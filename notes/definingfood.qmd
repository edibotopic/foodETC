...
- **Technology**: any system designed by people to serve a function, usually with the aid of scientific knowledge
:::
...
As societies shift from a context of food scarcity to one of food abundance, there can be increased opportunities to focus on other activities, including leisure, creativity and study.
...
Despite the fact that we all eat, there is not universal agreement on what food *is*, what is *good* food and what might *become* food.
...
Dystopianism often coexists with a `romantic` vision of the past, a better time when we were closer to nature.
...
Thus, instead of doing experiments we have to reason carefully about the ideas.
Most sciences are highly specialised, and it is obvious in a specific science what people mean by `parts`; for example, in physics this might be sub-atomic particles.
When doing ontology we can think of parts more generally, in a way that spans different disciplines.
A social scientist does not typically care about sub-atomic particles, as the fundmanental parts (individual people) are different in the systems (societies) that they study.
As food science is multi-disciplinary (chemistry, physics, biology, sensory science, etc.) and applied (it is used to design technology) the question *what parts do food scientists study?* is not easy.
...
Nonetheless, it is not uncommon to hear scientists refer to such concepts, especially when develop technology.
...
Furthermore, given that the seed may have been purposefully planted and its environment manipulated to promote its growth, there is a also a question as to whether the entire process might be artificial.
:::
