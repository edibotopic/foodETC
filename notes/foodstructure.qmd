...
Note: there are no slides associated with this set of notes, which themselves contain a variety of interactive elements.
...
## Milk Structure as an Emulsion (click-through animation)

[🥛 Full-screen animation of milk structure](../slides/six/sozi/emulsion.sozi.html)
...
Consider the table below. Imagine you are asked which of the foods you expect to be most "water-like".
Intuitively, you might expect that the system that contains the most water would be most like water.
However, this neglects to consider how that water might be structured (is it free or bound? is it liquid or solid?).
...
- Superposition (σ): one thing on top of another
- Inclusion (@): one thing inside of another
- Disconnection (+): one thing disconnected to another
- Bicontinuous (×): two things that are continuous with each other
...
This would mean "x and y are both disconnected from each other, but both are included in z".
...
The models can be downloaded and 3D-printed.
...
[🥩 Click here for full-screen animation of meat structure](../slides/six/sozi/meat.sozi.html)
